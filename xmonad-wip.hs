...
chatLayout = smartBorders(avoidStruts(magnifiercz' 1.3 $ multiCol [1] 1 0.01 (-0.5)))
...
kbStartupHook = do
  ...
  when (null as) $ do
    ...
    void $ startProcess "microsoft-edge"
--}
...
xmonadConfig =
  ...
  def
    { manageHook = manageDocks <+>
      myManageHook <+>
      namedScratchpadManageHook  kbScratchPads <+> manageHook def
    -- No red border for media players
    , terminal = myTerminal
--    , startupHook = kbStartupHook
